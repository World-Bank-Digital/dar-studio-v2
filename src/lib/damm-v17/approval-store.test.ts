...
describe("post-completion human approval store", () => {
  ...
  it("keeps the 0028 source-pin package immutable and audit-readable after the 0029 cutover", async () => {
    let fx = await fixture("historical-0029-complete");
    try {
      fx = await makePackageHistorical(
        fx,
        PRE_0029_DAMM_SOURCE_COMMIT,
        PRE_0029_DAMM_RENDERER_SHA256,
      );
      const beforeCutover = await approvalAuditSnapshot(fx);
      await fx.pg.exec(
        await readFile(
      ...
      assert.deepEqual(
        await approvalAuditSnapshot(fx),
        beforeCutover,
        "the 0029 cutover must not rewrite a completed preceding-pin package or its artifacts",
      );

      const reopened = unwrap(await ensureApprovalPackage(fx.countryId, USERS.owner.id, fx.sql));
      assert.equal(reopened.id, fx.approvalPackage.id);
      const historical = unwrap(await getOwnerApprovalState(fx.countryId, USERS.owner.id, fx.sql));
      assert.equal(historical.package.methodology.sourceCommit, PRE_0029_DAMM_SOURCE_COMMIT);
      assert.equal(historical.package.methodology.rendererSha256, PRE_0029_DAMM_RENDERER_SHA256);
      assert.equal(historical.packageHistory[0].currentMethodology, false);
      assertMethodologyRefusal(
        await assignApprovalReviewer(
          {
