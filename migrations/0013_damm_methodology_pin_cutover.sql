-- Cut active workflows and new approval packages over to the DAMM source revision
-- whose Stage 8 package contains the exact Stage 1 engine input and whose automated
-- challenge is not represented as human G1/G2. Existing terminal Draft packages and
-- their immutable approval records remain untouched.
  ...
  if exists (
    ...
    where workflow_run.pass = 'workflow'
      ...
      and (
        ...
        or methodology.source_commit is distinct from '92c6ffe8b331347bc05f345785fe409753401a24'
    ...
    raise exception using
      errcode = '55000',
      message = 'Cannot install the current DAMM methodology pin while stale or missing-pin workflows are active; allow them to finish and retry the deployment.';
  end if;
end;
$$;
  ...
  if new.pass = 'workflow'
     ...
     and not exists (
       ...
       where methodology.run_id = new.id
         ...
         and methodology.source_commit = '92c6ffe8b331347bc05f345785fe409753401a24'
    ...
    raise exception 'A nonterminal workflow run requires the current DAMM methodology pin.'
      using errcode = '55000';
  end if;
  return new;
end;
...
begin
  ...
  where workflow_run.id = new.run_id
    ...
    and new.damm_source_commit = '92c6ffe8b331347bc05f345785fe409753401a24'
