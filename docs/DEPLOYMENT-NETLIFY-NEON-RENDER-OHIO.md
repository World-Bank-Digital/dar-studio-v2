...
6. **Migration 0028 follows 0019 through 0027 before the repinned Render worker deploys.** Take a Neon recovery snapshot, prove there are no active workflows, run migrations with the direct Neon connection, and verify exactly one ledger row for each current migration. `0019_progressive_stage_artifacts.sql` installs transactionally sealed, owner-only Stage 1–7 publications without historical backfill; `0020`–`0024` retain the previously documented source-pin hardening. `0028_damm_source_pin_cutover.sql` advances only the current source pin to DAMM PR #17 merge `3cd0c599ff137a09a1892b498f0eecfca5f43785`, adding bounded Exa Contents retrieval for citation-only sources, exact source identity checks, conservative settlement and durable paid-request replay protection. The model, schema, workflow, engine, renderer and ratification fields remain unchanged; source pricing adds the bounded Exa Contents charge. Its 38-file production dependency closure is `f6d501249f0b448a8090406ac6829f2a55203582f791cbd1561ed3e24a313e13`. Only after all seven gates pass may the Render worker and artifact gateway deploy; neither service runs migrations.
...
### Current 0028 source-pin release invariant

The detailed `0024` descriptions below remain the immutable record of that
earlier cutover. For the current reviewed release candidate, `0028` is the
operative source-pin cutover and supersedes any prior current-pin/count wording
in this runbook. It advances the canonical source to DAMM PR #17 merge
`3cd0c599ff137a09a1892b498f0eecfca5f43785` and preserves the model, schema,
workflow, engine, renderer and ratification identity. The reviewed repair adds a
bounded Exa Contents request for citation-only sources, exact response identity
checks and durable conservative settlement. The source pricing file adds the
$0.001 one-page Contents bound. URL-changing redirects stop conservatively. The
...
root-production snapshot named `pre-0028-...`, and a clean merged DAR commit.
After it, require exactly 28 migration-ledger rows through
`0028_damm_source_pin_cutover.sql`, the exact `3cd0c59...` database guard, and
all six protected Nigeria failures unchanged, including b481ddea (0/8, $13.35446735) and fcc17f6c (0/8, $14.8487319). Preserve `7d623f0...` with its
...
shallow-fetch and verify `3cd0c59...` itself (one reachable commit, no tags,
...
5. create the non-expiring root-production snapshot before applying `0028`.

After `0028` verifies, keep the already-disconnected Blueprint disconnected; do
...
### 7. Apply and verify migration 0028 after 0019 through 0027
...
`f6d501249f0b448a8090406ac6829f2a55203582f791cbd1561ed3e24a313e13`.
If `0028` reports the quoted stale/missing-pin guard, allow the active workflow
...
Require exactly ten rows, one for each filename in that order. Verify that the
...
select pg_get_functiondef('require_active_workflow_methodology()'::regprocedure)
  like '%3cd0c599ff137a09a1892b498f0eecfca5f43785%' as pinned_commit,
...
Also require the whole migration ledger to contain exactly 28 rows through
`0028`, repeat the active-workflow query and require zero rows, and prove the six
...
Require `28` and `0028_damm_source_pin_cutover.sql`. Require all six runs to remain
...
`0028` are verified. For an existing environment whose Blueprint is already
...
7. Refresh both bound source identities immediately before clicking: require local DAR `HEAD`, `origin/main`, and the direct GitHub branch identity to equal the recorded `DEPLOY_GIT_SHA`. In an empty temporary home with inherited/global/system Git configuration and interactive credential paths disabled, anonymously shallow-fetch and verify exact DAMM commit `3cd0c599ff137a09a1892b498f0eecfca5f43785`. Require a clean one-commit/no-tag checkout and strict object integrity. Record DAMM `refs/heads/main` only as diagnostic information; it may advance and must not be compared with the immutable manifest pin. If the exact pin cannot be fetched or verified, stop before deployment and restart from a reviewed source state.
...
- the DAMM checkout is exactly commit `3cd0c599ff137a09a1892b498f0eecfca5f43785`;
...
- preflight reports exactly `migrations=28`, after the SQL verification above
  proved one ledger row through `0028` and no unexpected row;
...
- the migration output is `up to date` against the already verified exact
  28-row ledger through `0028`; it must not be the first process to apply `0028`;
...
commit on Netlify, the gateway, and the worker; exact DAMM `3cd0c599...`; worker
`node=22.22.3 python=3.12.13 migrations=28`; Basic-protection anonymous denial
...
zero active workflows; the `0028` guard; and the unchanged six failed-run rows.
...
- exact reviewed DAR on Netlify, gateway, and worker; exact DAMM
  `3cd0c599ff137a09a1892b498f0eecfca5f43785`; 38-file identity
  `f6d501249f0b448a8090406ac6829f2a55203582f791cbd1561ed3e24a313e13`;
  and the expected model/renderer hashes;
- one Live worker instance, one possible claimant, a confirmed lease margin,
  migration `0028`, zero active workflows, no unresolved spend reservation, and
  all six failed runs unchanged; and
