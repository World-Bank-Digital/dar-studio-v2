...
6. **Migration 0015 precedes the repinned Render worker.** Take a Neon recovery snapshot, prove there are no active workflows, run migrations with the direct Neon connection, verify exactly one migration ledger row for `0015_damm_source_pin_cutover.sql`, and only then deploy the Render worker and artifact gateway. This append-only cutover advances the worker to the canonical DAMM merge that fixes Stage 6 partial-range validation; neither Render service runs migrations.
...
| `MIGRATION_0015_VERIFIED` | exact Neon ledger/function checks | local deployment record | integrity evidence; fixed `true` only after verification |
...
### 7. Apply and verify migrations, including 0015
...
The migrator applies sorted SQL files one at a time, each in its own transaction, and records the filename in `_migrations`. A failure rolls back that file. Migrations `0013` and `0014` remain immutable historical evidence; migration `0015` performs the new source-pin cutover to the canonical DAMM merge containing the Stage 6 partial-range validation fix. If migration 0015 reports:
...
from _migrations
where name = '0015_damm_source_pin_cutover.sql';
```
...
select pg_get_functiondef('require_active_workflow_methodology()'::regprocedure)
  like '%2efb26607acc29a687a82a56edc85f53c4a6da69%' as pinned_commit,
...
- the DAMM checkout is exactly commit `2efb26607acc29a687a82a56edc85f53c4a6da69`;
