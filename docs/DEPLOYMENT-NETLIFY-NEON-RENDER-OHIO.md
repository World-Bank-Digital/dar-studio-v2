...
| `MIGRATION_0014_VERIFIED` | exact Neon ledger/function checks | local deployment record | integrity evidence; fixed `true` only after verification |
...
4. Confirm the merged tree contains `netlify.toml`, `render.yaml`, both `Dockerfile.worker` and `Dockerfile.artifact-gateway`, the worker entrypoint/preflight files, `deploy/artifact-gateway/package.json` and its lockfile, `scripts/artifact-gateway.ts`, immutable predecessor migration `0013_damm_methodology_pin_cutover.sql`, current migration `0014_damm_source_pin_cutover.sql`, and this runbook.
...
### 7. Apply and verify migrations, including 0014
...
The migrator applies sorted SQL files one at a time, each in its own transaction, and records the filename in `_migrations`. A failure rolls back that file. Migration `0013` remains immutable historical evidence; migration `0014` performs the new source-pin cutover. If migration 0014 reports:
...
from _migrations
where name = '0014_damm_source_pin_cutover.sql';
```
...
select pg_get_functiondef('require_active_workflow_methodology()'::regprocedure)
  like '%d4c659f5873f3a891634c8edf6b7166cb2eb374c%' as pinned_commit,
...
- the DAMM checkout is exactly commit `d4c659f5873f3a891634c8edf6b7166cb2eb374c`;
