...
6. **Migration 0016 precedes the repinned Render worker.** Take a Neon recovery snapshot, prove there are no active workflows, run migrations with the direct Neon connection, verify exactly one migration ledger row for `0016_damm_source_pin_cutover.sql`, and only then deploy the Render worker and artifact gateway. This append-only cutover advances the worker to the canonical DAMM merge with bounded adaptive Stage 6 output and crash-safe spend/checkpoint recovery; neither Render service runs migrations.
...
| `MIGRATION_0016_VERIFIED` | exact Neon ledger/function checks | local deployment record | integrity evidence; fixed `true` only after verification |
...
4. Confirm the merged tree contains `netlify.toml`, `render.yaml`, both `Dockerfile.worker` and `Dockerfile.artifact-gateway`, the worker entrypoint/preflight files, `deploy/artifact-gateway/package.json` and its lockfile, `scripts/artifact-gateway.ts`, immutable historical migrations `0013_damm_methodology_pin_cutover.sql`, `0014_damm_source_pin_cutover.sql`, and `0015_damm_source_pin_cutover.sql`, current migration `0016_damm_source_pin_cutover.sql`, and this runbook.
...
### 7. Apply and verify migrations, including 0016
...
The migrator applies sorted SQL files one at a time, each in its own transaction, and records the filename in `_migrations`. A failure rolls back that file. Migrations `0013`, `0014`, and `0015` remain immutable historical evidence; migration `0016` performs the new source-pin cutover to the canonical DAMM merge containing bounded adaptive Stage 6 provider calls, durable checkpoints, and crash-safe spend recovery. If migration 0016 reports:
...
from _migrations
where name = '0016_damm_source_pin_cutover.sql';
```
...
select pg_get_functiondef('require_active_workflow_methodology()'::regprocedure)
  like '%1b1734c8a8017cda488b77cf0594b0ca82dae6ee%' as pinned_commit,
...
- the DAMM checkout is exactly commit `1b1734c8a8017cda488b77cf0594b0ca82dae6ee`;
