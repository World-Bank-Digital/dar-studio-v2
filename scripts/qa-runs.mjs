...
try {
  ...
  await page.getByRole("button", { name: "Research", exact: true }).click();
  await page
    .getByRole("button", { name: /Import into the workspace/i })
    .first()
    .click();
  ...
  const heldRow = await page
    .locator("tr")
    .filter({ hasText: /^1\.1yours|1\.1/ })
    .filter({ hasText: /77\.7/ })
    .first()
    .innerText();
  if (!/77\.7/.test(heldRow))
    throw new Error(`the held row did not show the assessor's value: ${heldRow}`);
  ...
  // --- legacy automated vendor challenge (machine QC, never human G1/G2) -------
  await page.selectOption("select >> nth=1", "g2");
  await page.locator("input").first().fill("200");

  // Same vendor family as the research pass. This must be refused: a model that
  // machine-checks its own family can report a clean result it never earned.
  await page.selectOption("select >> nth=2", "anthropic/claude-sonnet-5");
  await page.getByRole("button", { name: /^Start$/ }).click();
  // Assert on wording only the refusal produces. Matching a standing explanatory hint
  // would pass whether or not the rule fired — a test that cannot fail is worse than no
  // test.
  await page.getByText(/Cannot start the automated vendor challenge/i).waitFor({ timeout: 15000 });
  await page.screenshot({ path: `${shot}/runs-5-peer-refused.png` });
  const refusal = await page.getByText(/Cannot start the automated vendor challenge/i).innerText();
  if (!/anthropic/i.test(refusal)) throw new Error(`refusal did not name the vendor: ${refusal}`);
  const cards = await page.getByText(/Automated vendor challenge/).count();
  if (cards > 1) throw new Error("a run was created despite the refusal");
  step("refused same-family automated machine QC", { refusal });

  await page.selectOption("select >> nth=2", "openai/gpt-5.6-terra");
  await page.getByRole("button", { name: /^Start$/ }).click();
  step("started the automated challenge on another vendor");
  await page
    .getByText(/Finished .* rows/i)
    .nth(1)
  ...
  step("both passes recorded", {
    ...
    automatedChallenge: /Automated vendor challenge/.test(both),
  });
